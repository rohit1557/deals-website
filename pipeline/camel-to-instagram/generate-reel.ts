...
const SKIP_REEL = process.env.SKIP_REEL === "true";
...
export async function generateReel(): Promise<void> {
  ...
  if (DRY_RUN || SKIP_REEL) {
    console.log(`[generate-reel] ${DRY_RUN ? "DRY_RUN" : "SKIP_REEL"}=true — skipping Cloudinary upload and TryPost scheduling`);
