...
const AUTO_POST_INSTAGRAM = !DRY_RUN && process.env.POST_TO_INSTAGRAM === "true";
const AUTO_POST_BUFFER    = !DRY_RUN && process.env.POST_TO_BUFFER    === "true";
...
async function main() {
  ...
  if (DRY_RUN) {
    console.log("[pipeline] DRY_RUN=true — skipping OzBargain post");
  } else {
